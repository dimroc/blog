...
the ground truth density map is generated by taking the head annotations and setting that pixel value to one, and
then gaussian blurring the image. The model is then trained to output these density maps.
...
1. Use Vue JS and d3 to visualize the line count.
2. Create a docker image with your static assets and Conda dependencies.
3. Deploy to GCP with kubernetes on Google Container Engine.
4. Periodically run a background job to scrape the shake cam image and run a prediction.
